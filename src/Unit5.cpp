...
void __fastcall TForm5::Button3Click(TObject *Sender)
{
        if(CheckListBox1->Items->Count == 0){
                MessageDlg("Немає жодного стовпця",
                mtError, TMsgDlgButtons()<<mbOK,0);
        }
        else if(Edit1->Text == ""){
                MessageDlg("Не вказана кількість рядків",
                mtError, TMsgDlgButtons()<<mbOK,0);
        }
        else if(!is_int(Edit1->Text)){
                MessageDlg("Введено не ціле додатнє число",
                mtError, TMsgDlgButtons()<<mbOK,0);
        }
        else if(strlen(Edit1->Text.c_str()) > 4){
                MessageDlg("Число повинно бути меншим за 10000",
                mtError, TMsgDlgButtons()<<mbOK,0);
        }
        else if(StrToInt(Edit1->Text)==0){
                MessageDlg("Число повинно бути більшим за 0",
                mtError, TMsgDlgButtons()<<mbOK,0);
        }
        else{
