...
if(shown_db.cols[index].datatype == "string"){
        ComboBox2->Items->Add("Від А до Я");
        ComboBox2->Items->Add("Від Я до А");
}
else if(shown_db.cols[index].datatype == "int" || shown_db.cols[index].datatype == "double"){
        ComboBox2->Items->Add("Зростання");
        ComboBox2->Items->Add("Спадання");
}
else if(shown_db.cols[index].datatype == "bool"){
        ComboBox2->Items->Add("Спочатку true");
        ComboBox2->Items->Add("Спочатку false");
}
}
//---------------------------------------------------------------------------
